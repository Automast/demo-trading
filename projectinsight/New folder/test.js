...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Status options for deposits and withdrawals
  const DEPOSIT_STATUSES = ['pending', 'confirmed', 'rejected', 'canceled'];
  const WITHDRAWAL_STATUSES = ['pending', 'confirmed', 'canceled', 'rejected'];
  const VERIFICATION_STATUSES = ['not_verified', 'pending', 'verified', 'rejected'];
  ...
  function populateUsersTable(users) {
    ...
    users.forEach(user => {
      ...
      tr.innerHTML = `
        ...
        <td>
          <select class="status-dropdown" data-user-id="${user.id}" data-field="verificationStatus">
            ${VERIFICATION_STATUSES.map(status => 
              `<option value="${status}" ${user.verificationStatus === status ? 'selected' : ''}>
  ...
  function displayUserDetails(data) {
    ...
    userInfoSection.innerHTML = `
      ...
      <table>
        <tbody>
          ...
          <tr><td>Account Currency</td><td>${user.accountCurrency}</td></tr>
          <tr><td>Fiat Balance</td><td>${user.fiatBalance || '0'} ${user.accountCurrency} <button class="button edit" data-field="fiatBalance">Edit</button></td></tr>
          <tr><td>Verification Status</td><td>
            <select class="admin-status-dropdown" data-table="users" data-id="${user.id}" data-field="verificationStatus">
              ${VERIFICATION_STATUSES.map(status => 
                `<option value="${status}" ${user.verificationStatus === status ? 'selected' : ''}>
                  ${status.charAt(0).toUpperCase() + status.slice(1).replace('_', ' ')}
                </option>`
    ...
    // Deposits with Status Dropdowns
        ...
        <tbody>
          ${deposits.map(deposit => `
            <tr>
              ...
              <td>
                <select class="admin-status-dropdown" data-table="deposits" data-id="${deposit.id}" data-user-id="${user.id}" data-field="status">
                  ${DEPOSIT_STATUSES.map(status => 
                    `<option value="${status}" ${deposit.status === status ? 'selected' : ''}>
    ...
    // Withdrawals with Status Dropdowns
        ...
        <tbody>
          ${withdrawals.map(withdrawal => `
            <tr>
              ...
              <td>
                <select class="admin-status-dropdown" data-table="withdrawals" data-id="${withdrawal.id}" data-user-id="${user.id}" data-field="status">
    ...
    // Add event listeners for all admin status dropdowns
      ...
      dropdown.addEventListener('change', async (e) => {
        ...
        try {
          ...
          if (table === 'users') {
            ...
            url = `/api/user/${userId}/deposits/${id}`;
          } else if (table === 'withdrawals') {
            url = `/api/user/${userId}/withdrawals/${id}`;
          } else {
          ...
          if (response.ok) {
            ...
            // Refresh user details to show any balance updates
            if (table === 'deposits' || table === 'withdrawals') {
              setTimeout(() => viewUserDetails(userId), 1000);
            }
          } else {
            throw new Error('Failed to update status');
          }
  ...
  async function openEditModal(table, id, field) {
    try {
      ...
      if (field) {
        ...
        if (field === 'status' && table === 'deposits') {
          editFormFields.innerHTML += `
            <div class="form-group">
              <label for="field-value">${field}</label>
              <select id="field-value" name="fieldValue">
                ${DEPOSIT_STATUSES.map(status => 
          ...
          `;
        } else if (field === 'status' && table === 'withdrawals') {
          editFormFields.innerHTML += `
            <div class="form-group">
              <label for="field-value">${field}</label>
              <select id="field-value" name="fieldValue">
  ...
  editForm.addEventListener('submit', async (e) => {
    ...
    let payload = {};
    if (editFormFields.querySelector('input[name="fieldValue"]') || editFormFields.querySelector('select[name="fieldValue"]')) {
      // Editing a single field
      const fieldElement = editFormFields.querySelector('input[name="fieldValue"]') || editFormFields.querySelector('select[name="fieldValue"]');
  ...
  async function openAddModal(table, userId) {
    try {
      ...
      switch (table) {
        case 'user_wallets':
          fields = ['coinName', 'shortName', 'walletAddress', 'privateKey', 'balance', 'type'];
          break;
        ...
        case 'user_stakes':
          ...
          break;
        case 'deposits':
          fields = ['date', 'reference', 'method', 'type', 'amount', 'totalEUR', 'status'];
          break;
        case 'withdrawals':
          fields = ['date', 'reference', 'method', 'amount', 'total', 'status', 'type'];
          break;
        case 'notifications':
      ...
      fields.forEach(field => {
        if (field === 'status' && table === 'deposits') {
          editFormFields.innerHTML += `
            <div class="form-group">
              ...
              <select id="${field}" name="${field}">
                ${DEPOSIT_STATUSES.map(status => 
                  `<option value="${status}" ${status === 'pending' ? 'selected' : ''}>
                    ${status.charAt(0).toUpperCase() + status.slice(1)}
                  </option>`
                ).join('')}
              </select>
          ...
          const defaultValue = field === 'isRead' ? '0' : 
                              field === 'date' ? new Date().toISOString() : 
                              field === 'createdAt' ? new Date().toISOString() : 
                              field === 'type' && table === 'user_wallets' ? 'crypto' : '';
          editFormFields.innerHTML += `
            <div class="form-group">
              <label for="${field}">${field}</label>
              <input type="text" id="${field}" name="${field}" value="${defaultValue}">
            </div>
          `;
        }
      });
